	...
	<c:if test="${liveMode}">
		...
		<template:addResources>
				...
				$.ajax({
					...
					success: function (data, textStatus, xhr) {
						...
						if (data.status == "ok") {
                            ...
                            $('#subscribeFormPanel-${currentNode.identifier}-${target.propertiesAsString['jcr:uuid']}').effect("shake", {times:4}, 60);
                            <fmt:message key="messsage.subscriptions.invalidEmailAddress" var="msg"/>
                            $('#subscribeError-${currentNode.identifier}-${target.propertiesAsString['jcr:uuid']}').html("${functions:escapeJavaScript(msg)}")					    	
					        doClose=false;
			...
			$(document).ready(function() {
		        ...
		        $("#subscribeForm-${currentNode.identifier}-${target.propertiesAsString['jcr:uuid']}").submit(function() {
		        ...
		        $("#unsubscribeForm-${currentNode.identifier}-${target.propertiesAsString['jcr:uuid']}").submit(function() {
		        	if (this.email.value.length == 0) {
				    	<fmt:message key="messsage.subscriptions.provideEmailAddress" var="msg"/>
                        $('#unsubscribeFormPanel-${currentNode.identifier}-${target.propertiesAsString['jcr:uuid']}').effect("shake", {times:4}, 60)
                        $('#unsubscribeError-${currentNode.identifier}-${target.propertiesAsString['jcr:uuid']}').html("${functions:escapeJavaScript(msg)}")
			        	return false;
		        	}
		...
		<c:if test="${not renderContext.loggedIn}">
			...
			&nbsp;<a href="#subscribeFormPanel-${currentNode.identifier}-${target.propertiesAsString['jcr:uuid']}" rel="${currentNode.identifier}-${target.propertiesAsString['jcr:uuid']}" class="showSubscriptionForm" title="<fmt:message key='label.subscribe'/>"><img src="<c:url value='${url.currentModule}/icons/jnt_subscriptions.png'/>" alt="<fmt:message key='label.subscribe'/>" title="<fmt:message key='label.subscribe'/>" height="16" width="16"/><fmt:message key='label.subscribe'/></a>
			&nbsp;<a href="#unsubscribeFormPanel-${currentNode.identifier}-${target.propertiesAsString['jcr:uuid']}" rel="${currentNode.identifier}-${target.propertiesAsString['jcr:uuid']}" class="showSubscriptionForm" title="<fmt:message key='label.unsubscribe'/>"><img src="<c:url value='${url.currentModule}/icons/unsubscribe.png'/>" alt="<fmt:message key='label.unsubscribe'/>" title="<fmt:message key='label.subscribe'/>" height="16" width="16"/><fmt:message key='label.unsubscribe'/></a>
			</p>
			<div class="jahiaFancyboxForm">
			<div id="subscribeFormPanel-${currentNode.identifier}-${target.propertiesAsString['jcr:uuid']}" style="width: 500px; height:500px;">
			    <div class="popup-bodywrapper">
			        <h3 class="boxmessage-title"><fmt:message key='label.subscribe'/>&nbsp;${subscribeTitle}</h3>
                    <template:tokenizedForm>
			        <form class="formMessage jahiaSubscribeForm" id="subscribeForm-${currentNode.identifier}-${target.propertiesAsString['jcr:uuid']}" method="post" action="<c:url value='${url.base}${target.path}.subscribe.do' />">
                        <c:forEach items="${currentNode.properties['j:fields']}" var="fld">
                            <input type="hidden" name="j:fields" value="${fld.string}" />
                        </c:forEach>
			            <fieldset>
			                <p>
			                	<label for="subscribeForm-${currentNode.identifier}-${target.propertiesAsString['jcr:uuid']}-email" class="left"><fmt:message key="label.email"/>*</label>
			                    <input type="text" name="email" id="subscribeForm-${currentNode.identifier}-${target.propertiesAsString['jcr:uuid']}-email" class="field" value="" tabindex="20" size="40"/><span id="subscribeError-${currentNode.identifier}-${target.propertiesAsString['jcr:uuid']}" style="color:red;"></span>
			                </p>
			                <c:forEach items="${currentNode.properties['j:fields']}" var="fld" varStatus="status">
	                			<c:set var="fldKey" value="${fn:replace(fld.string, ':', '_')}"/>
				                <p>
				                	<label for="subscribeForm-${currentNode.identifier}-${target.propertiesAsString['jcr:uuid']}-${fldKey}" class="left"><fmt:message key="jnt_subscriptionComponent.j_fields.${fldKey}"/></label>
				                    <input type="text" name="${fld.string}" id="subscribeForm-${currentNode.identifier}-${target.propertiesAsString['jcr:uuid']}-${fldKey}" class="field" value="" tabindex="${20 + status.count}" size="30"/>
				                </p>
			                </c:forEach>
                             <p class="field">
			                ...
			                <input class="button" type="button" value=" <fmt:message key="label.subscribe"/> "
			                       tabindex="30" onclick="$('#subscribeForm-${currentNode.identifier}-${target.propertiesAsString['jcr:uuid']}').submit();">
<!-- 			                       tabindex="30" onclick="test();"> -->
			            </fieldset>
			        </form>
                    </template:tokenizedForm>
			    </div>
			...
			<div class="jahiaFancyboxForm">			
			<div id="unsubscribeFormPanel-${currentNode.identifier}-${target.propertiesAsString['jcr:uuid']}" style="width: 350px;height: 150px;">
			    <div class="popup-bodywrapper">
			        <h3 class="boxmessage-title"><fmt:message key='label.unsubscribe'/>&nbsp;${subscribeTitle}</h3>
                    <template:tokenizedForm>
			        <form class="formMessage jahiaUnsubscribeForm" id="unsubscribeForm-${currentNode.identifier}-${target.propertiesAsString['jcr:uuid']}" method="post" action="<c:url value='${url.base}${target.path}.unsubscribe.do'/>">
			            <input type="hidden" name="j:to" id="destinationUserKey" value="" />
			            <fieldset>
			                <p>
			                	<label for="subscribeForm-${currentNode.identifier}-${target.propertiesAsString['jcr:uuid']}-email" class="left"><fmt:message key="label.email"/>*</label>
			                    <input type="text" name="email" id="subscribeForm-${currentNode.identifier}-${target.propertiesAsString['jcr:uuid']}-email" class="field" value="" tabindex="20" size="40"/><span id="unsubscribeError-${currentNode.identifier}-${target.propertiesAsString['jcr:uuid']}" style="color:red;"></span>
			                </p>
                            <p class="field">
                                <label class="left" for="captcha">${fn:escapeXml(jcr:label(props.captcha.definition,currentResource.locale))}</label><template:captcha />
                                <label class="left" for="inputCaptcha"></label><input type="text" id="inputCaptcha" name="jcrCaptcha"/>
			                ...
			                <input class="button" type="button" value=" <fmt:message key="label.unsubscribe"/> "
			                       tabindex="30" onclick="$('#unsubscribeForm-${currentNode.identifier}-${target.propertiesAsString['jcr:uuid']}').submit();">
			            </fieldset>
			        </form>
                    </template:tokenizedForm>
			    </div>
